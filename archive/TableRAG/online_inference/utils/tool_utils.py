...
class Embedder :
    def __init__(self, model_path, device_id=1) -> None:
        ...
        if torch.cuda.is_available():
            self.device = torch.device(f"cuda:{min(device_id, torch.cuda.device_count() - 1)}")
        else:
            self.device = torch.device("cpu")
        self.tokenizer: Any = AutoTokenizer.from_pretrained(self.model_path, use_fast=True)
...
class Reranker :
    def __init__(
        ...
        self.tokenizer: Any = AutoTokenizer.from_pretrained(model_name_or_path, cache_dir=cache_dir)
        ...
        if device and isinstance(device, str) :
            ...
            if device is not None :
                self.device = torch.device(f"cuda:{min(int(device), torch.cuda.device_count() - 1)}")
            else :
    ...
    @torch.no_grad()
    def compute_score(self, sentence_paris, batch_size: int = 256,
