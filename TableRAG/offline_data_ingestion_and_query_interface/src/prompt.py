...
Based on the schemas above, please use SQLite syntax to solve the following problem:
{user_query}
Please wrap the generated SQL statement with ```sql ```, and wrap table name and each column name mentioned in sql with ``, for example: ```sql SELECT `name` FROM `student_sheet1` WHERE `age` > '15';```
'''