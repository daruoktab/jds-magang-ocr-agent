...
class TableRAG() :
    ...
    def relate_to_table(self, doc_name: str) -> str :
        ...
        """
        if "json" in doc_name :
            return f"[\"{doc_name.replace('.json', '_sheet1.xlsx')}\"]"
        return ""
    ...
    def extract_subquery(self, response: Any, backbone: str = 'openai') -> Tuple[str, Any, Any] :
        """
    ...
    def get_llm_response(self, text_messages: Any, tools: Any, backbone: str, select_config: Any) :
        if tools :
        ...
        while current_iter :
            ...
            for sub_query, tool_call_id in zip(sub_queries, tool_call_ids) :
                ...
                try :
                    ...
                    schema  = excel_rag_response['nl2sql_prompt'].split('Based on the schemas above, please use SQLite syntax to solve the following problem')[0].strip()
                except Exception as e :
                    sql_str = sql_execute_result = schema = f"ExcelRAG execute fails: {e}"
    ...
    def run(
        ...
        if rerun :
            ...
            pre_questions = {case["question"]: case for case in pre_data}
